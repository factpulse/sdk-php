...
```php
getVerificationTypedStatusApiV1VerificationVerifyAsyncTaskIdTypedStatusGet($task_id): \FactPulse\SDK\Model\VerificationTypedTaskStatus
```
...
try {
    $result = $apiInstance->getVerificationTypedStatusApiV1VerificationVerifyAsyncTaskIdTypedStatusGet($task_id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling FacturXPDFXMLVerificationApi->getVerificationTypedStatusApiV1VerificationVerifyAsyncTaskIdTypedStatusGet: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\FactPulse\SDK\Model\VerificationTypedTaskStatus**](../Model/VerificationTypedTaskStatus.md)
